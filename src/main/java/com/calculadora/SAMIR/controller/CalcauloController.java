...
public class CalcauloController {
	...
	public @ResponseBody Object calcular(@RequestBody InfoCalculoDTO informacoes) {
		try {
			...
			// LocalDate dataComparacao = LocalDate.of(anoCalculo, mesCalculo, 1);
			// if (informacoes.isJuros()) {
			// 	if (informacoes.isSelic()
			// 			&& (selicDate.isBefore(LocalDate.of(anoCalculo, mesCalculo, 1)))) {
			...
			if (anoCalculo <= anoDip) {
				...
				while (anoCalculo != anoDip + 1) {
					...
					CalculoDTO calculoAdd = new CalculoDTO(dataCalculo, reajuste, rmi, correcaoAcumulada,
							jurosAcumulado,
							porcentagemRmi);
